...
#Use Filter Function
df=df[(df["date"] >= 20190115)&(df["date"] <= 20190121)]

df["date"].value_counts(dropna=False).sort_index()

df_revenues = df.groupby(['treat',"date"])[["revenues"]].sum().reset_index()
df_revenues = df_revenues.pivot(index = ["date"], values = "revenues", columns = ["treat"]).reset_index()
df_revenues["diff"] = df_revenues["Treatment"]-df_revenues["Control"]
df_revenues

df_revenues = df.groupby(['treat',"date","deviceCategory"])[["revenues"]].sum().reset_index()
df_revenues = df_revenues.pivot(index = ["date","deviceCategory"], values = "revenues", columns = ["treat"]).reset_index()
df_revenues["diff"] = df_revenues["Treatment"]-df_revenues["Control"]
df_revenues
...
# Write csv
df_revenues.to_csv(os.path.join(main_dir,data_files,"ab_test_revenues_wk_3.csv"))  
...
# Write csv
df_graph.to_csv(os.path.join(main_dir,data_files,"ab_test_cr_wk_3.csv"))  
