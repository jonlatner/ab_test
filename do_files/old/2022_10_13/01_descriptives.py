...
p = (ggplot(df_graph, aes(x = 'treat', y='value', label="value"))
+ geom_text(nudge_x=0.3, format_string='{:.4f}% ')
+ geom_point()   
...
df_revenues = df.groupby(['treat'])[["transactionRevenue"]].sum().reset_index()
df_revenues["transactionRevenue"]=df_revenues["transactionRevenue"]/1000
df_revenues["type"]=" Revenues (1000s)"
df_revenues = df_revenues.rename(columns={"transactionRevenue":"value"})
df_revenues
...
p = (ggplot(df_revenues, aes(x = 'treat', y = "value", label = "value"))
